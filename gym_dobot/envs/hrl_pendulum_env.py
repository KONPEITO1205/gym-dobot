import numpy as np
from gym_dobot.envs import rotations, robot_pendulum_env, utils, mjremote
...
class DobotPendulumEnv(robot_pendulum_env.RobotPendulumEnv):
    ...
    def __init__(
        ...
        super(RobotPendulumEnv, self).__init__(
    ...
    def _get_obs(self):
        obs = np.concatenate([np.cos(self.sim.data.qpos),np.sin(self.sim.data.qpos),
                               self.sim.data.qvel])
        achieved_goal = obs
        return {
    ...
    def _sample_goal(self):
        ...
        end_goal = np.zeros((len(self.goal_space_train)))
    ...
    def _is_success(self, achieved_goal, desired_goal):
        ...
        for i in range(len(achieved_goal)):
            if np.absolute(achieved_goal[i]- desired_goal[i]) > goal_thresholds[i]:
                return -1
        return 1
